printsumm <- function(soort,rnum,score_resp,score_feedb1,score_feedb2)
{
  ...
  feedb2 <- paste(as.character(score_feedb2)," %")
  png(filename=filename, width=1000, height=22,bg=bg) 
  par(mar=c(0.1,0.1,0,0.1),
      oma=c(0.1,0.1,0,0.1),
      layout(matrix(c(1,1,1,1,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,3,3,4,4,5,5),1,26)))
  usr <- par("usr")
  plot.new()
  ...
  plot.new()
  text(usr[1],usr[3],resp,adj = c(0,0),col=txtcol,cex=1.8)
  plot.new()
  text(usr[1],usr[3],feedb1,adj = c(0,0),col=txtcol,cex=1.8)
  plot.new()
  text(usr[1],usr[3],feedb2,adj = c(0,-0.3),col=txtcol,cex=1.8)
  graphics.off() 
}
