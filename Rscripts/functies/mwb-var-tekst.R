...
#
	...
	p12 <- updates[rapport,7]
        ...
        p64 <- updates[rapport,29]
        p65 <- updates[rapport,30]
        p71 <- updates[rapport,31]
        p72 <- updates[rapport,32]
        p73 <- updates[rapport,33]
        p74 <- updates[rapport,34]
        p75 <- updates[rapport,35]
        p76 <- updates[rapport,36]
        p81 <- updates[rapport,37]
  #
  # Nu gegevens van de facilitator ophalen
  #
  fac_record = dbGetQuery(mwb_aanvraag,statement=paste('select * from ',fac_table,' where fac_id=', "'",updates[rapport,5],"'", sep=""))
