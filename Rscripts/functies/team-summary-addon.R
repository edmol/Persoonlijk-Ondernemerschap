...
#
prio_run <- c(t9sum$prio[1],t9sum$prio[2],t9sum$prio[3])
section_tekst <- c(" "," "," ")
section_heading <- c(" "," "," ")
for(i in 1 : 3)
{
  ...
  if(prio_run[i] == 11)
  {
    section_tekst[i] <- "De kritieke succesfactor houding geeft inzicht in de wijze waarop jij, vanuit je lichaam en gedachten, je gedrag en gevoel stuurt. Het is DE graadmeter voor de mate waarin je in staat bent je te ontwikkelen en te transformeren."
  ...
  if(prio_run[i] == 12)
  {
    section_tekst[i] <- "De kritieke succesfactor gevoel geeft inzicht in de wijze waarop je, vanuit een `laag dieper` in jezelf, je gedrag en houding stuurt. Het is DE graadmeter voor de mate van betrokkenheid en ondernemendheid." 
  ...
  if(prio_run[i] == 13)
  {
    section_tekst[i] <- "De kritieke succesfactor organisatiekenmerk geeft inzicht op de wijze waarop, vanuit jouw perspectief en (zelf)beeld, jij, je team/organisatie jouw weerhoud dan wel stimuleert om te bewegen. Het is DE graadmeter voor de mate waarin jij en je team/organisatie is staat zijn tot duurzame verandering te komen." 
    section_heading[i] <- "Uitkomsten van de kritieke succesfactor Organisatiekenmerk" 
  ...
  if(prio_run[i] == 14)
  {
    section_tekst[i] <- "De kritieke succesfactor gedrag geeft inzicht in de wijze waarop jij, vanuit je handelen en (zelf)beeld, je doen en laten stuurt. Het is DE graadmeter voor de mate waarin je in staat bent je daadwerkelijk anders te zijn en anders te doen."
  ...
  if(prio_run[i] == 15)
  {
    section_tekst[i] <- "De kritieke succesfactor organisatie effect geeft inzicht in de korte termijn effecten van je intensies en waarden. Het is DE graadmeter voor de mate waarin je doet ook echt gebeurd vanuit wie je bent en waar je voor staat."
    section_heading[i] <- "Uitkomsten van de kritieke succesfactor Organisatie effect"
  ...
  if(prio_run[i] == 16)
  {
    ...
    img_prio[i,8] <-img[43]
    img_prio[i,9] <-img[63]
    img_prio[i,10] <-img[78]
    img_prio[i,11] <-img[94]
  ...
  if(prio_run[i] == 17)
  {
    ...
    img_prio[i,8] <-img[62]
    img_prio[i,9] <-img[76]
    img_prio[i,10] <-img[90]
    img_prio[i,11] <-img[92]
