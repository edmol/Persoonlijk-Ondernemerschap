...
$rapporttex = "team-".$soort."-tfo".'.tex';
$rapportpdf = "tfo-".$soort.'.pdf';
$rapporttoc = "tfo-".$soort.'.toc';
$htmldoc = "tfo";
//
...
$file = $dir.$rapporttex;
$newname = $orgnaam.'-'.'T'.$soort.'-'.$datum.'-compleet-.pdf';
...
if(file_exists($dir) && is_dir($dir))
{
	...
	{
	    chdir($dir);
		if($rapporttex=="team-01-tfo.tex")
		{
			system("pdflatex team-01-tfo.tex >/dev/null");
			system("pdflatex team-01-tfo.tex >/dev/null");
			rename("team-01-tfo.pdf", $newname);
			copy($newname,$rapdir.$newname); 
			header("Location: $htmlfile");
		}
		if($rapporttex=="team-02-tfo.tex")
		{
			system("pdflatex team-02-tfo.tex >/dev/null");
			system("pdflatex team-02-tfo.tex >/dev/null");
			rename("team-02-tfo.pdf", $newname);
			copy($newname,$rapdir.$newname); 
			header("Location: $htmlfile");
		}
if($rapporttex=="team-03-tfo.tex")
		{
			system("pdflatex team-03-tfo.tex >/dev/null");
			system("pdflatex team-03-tfo.tex >/dev/null");
			rename("team-03-tfo.pdf", $newname);
			copy($newname,$rapdir.$newname); 
			header("Location: $htmlfile");
		}
		if($rapporttex=="team-04-tfo.tex")
		{
			system("pdflatex team-04-tfo.tex >/dev/null");
			system("pdflatex team-04-tfo.tex >/dev/null");
			rename("team-04-tfo.pdf", $newname);
			copy($newname,$rapdir.$newname); 
			header("Location: $htmlfile");
		}
		if($rapporttex=="team-05-tfo.tex")
		{
			system("pdflatex team-05-tfo.tex >/dev/null");
			system("pdflatex team-05-tfo.tex >/dev/null");
			rename("team-05-tfo.pdf", $newname);
			copy($newname,$rapdir.$newname); 
			header("Location: $htmlfile");
		}
		if($rapporttex=="team-06-tfo.tex")
		{
			system("pdflatex team-06-tfo.tex >/dev/null");
			system("pdflatex team-06-tfo.tex >/dev/null");
			rename("team-06-tfo.pdf", $newname);
			copy($newname,$rapdir.$newname); 
			header("Location: $htmlfile");
		}
		if($rapporttex=="team-07-tfo.tex")
		{
			system("pdflatex team-07-tfo.tex >/dev/null");
			system("pdflatex team-07-tfo.tex >/dev/null");
			rename("team-07-tfo.pdf", $newname);
			copy($newname,$rapdir.$newname); 
			header("Location: $htmlfile");
		}
		if($rapporttex=="team-08-tfo.tex")
		{
			system("pdflatex team-08-tfo.tex >/dev/null");
			system("pdflatex team-08-tfo.tex >/dev/null");
			rename("team-08-tfo.pdf", $newname);
			copy($newname,$rapdir.$newname); 
			header("Location: $htmlfile");
		}
	} else 
