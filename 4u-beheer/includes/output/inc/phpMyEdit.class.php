...
/**
 * Hacks made to the distribution verion on 01 October 2007
 * $escape was missing from one function: function htmlTextarea()
 *    $escape = isset($this->fdd[$k]['escape']) ? $this->fdd[$k]['escape'] : true;
 * The generic2.css stylesheet contains image margin settings.
 * Use your stylesheet to set TD padding at 3px or 5px.
 * Commented out below: $this->sql_disconnect();
 * Commented out below: all <hr> tags
 * Changed below <h1> & <h4> converted to <h3>
 */
...
/* $Platon: phpMyEdit/phpMyEdit.class.php,v 1.208 2007-09-22 14:45:50 michal Exp $ */
...
class phpMyEdit
{
	...
	function myquery($qry, $line = 0, $debug = 0) /* {{{ */
	{
		...
		if ($debug_query || $debug) {
			$line = intval($line);
			echo '<h3>MySQL query at line ',$line,'</h3>',htmlspecialchars($qry),'<!-- <hr size="1" /> -->',"\n";
		}
		if (isset($this->db)) {
		...
		if (! $ret) {
			echo '<h3>MySQL error ',mysql_errno($this->dbh),'</h3>';
...
function '.$this->js['prefix'].'filter_handler(theForm, theEvent)
{
	...
	function htmlTextarea($name, $css, $k, $value = null, $js = NULL)
	{
      ...
      // Hack: The following line appears to be missing from CVS version as of 29 Sep 2007
		$escape = isset($this->fdd[$k]['escape']) ? $this->fdd[$k]['escape'] : true;
	...
	{
		echo '<h3>phpMyEdit error: ',htmlspecialchars($message),'</h3>',"\n";
		if ($additional_info != '') {
	...
	function execute() /* {{{ */
	{
		...
		// @ sign prepended as this is deprecated in later PHP versions
